...
def create_profile(
    ...
    *,
    ...
    repo_name
        ...
        Note: the filename may contain ``-`` (e.g.
        ``profile_DW-Production.py``), which is not a valid Python module
        name.  Sector scripts generated by
        :func:`cso_toolkit.create_sector_script` therefore load the
        profile via :func:`importlib.util.spec_from_file_location`
        rather than ``from <name> import ...``, so hyphenated profile
        filenames work without renaming.
    project_title

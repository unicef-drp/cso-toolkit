...
.cso_test_rules <- list(
  ...
  list(id = "io-xlsx",         family = "io",
       pattern = "(?<![a-zA-Z0-9_.])(?:readxl::|openxlsx::|writexl::)?(?:read_xlsx|read_excel|write_xlsx|write\\.xlsx|read\\.xlsx|getSheetNames|excel_sheets|loadWorkbook)\\s*\\(",
       message = "Direct Excel read / write / sheet-enumeration.",
       suggest = "dw_use(...) / dw_save(...) -- auto-dispatches on .xlsx; dw_use(sheet = NULL) reads all sheets (no getSheetNames loop)."),

...
## The datalib error taxonomy

The survey-microdata tool `datalib` uses a **typed** error taxonomy too, but with a **different envelope shape** from the toolkit's: it raises a stable class/code per failure — a Stata exit code, an R condition class, a Python exception — rather than the toolkit's `[cso_toolkit.<func>]` `WHAT` / `Why` / `Fix:` message prefix. The two are complementary, not interchangeable; do not merge them into one table.

{{< include ../includes/datalib/_error-taxonomy.md >}}

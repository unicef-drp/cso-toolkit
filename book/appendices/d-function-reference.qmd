...
::: {.callout-note}
The pkgdown *Reference* index is generated from the roxygen `@concept` tags, not hand-maintained. If you add an exported R function, tag it with the right `@concept` so it lands in the correct family on the site; an untagged export is still exported but is orphaned from the reference index.
:::
...
## Stata
```stata
dw_save using "dw_ed_edu.dta", sector(ed) kind(wrk)
dw_use using "dw_ed_edu.dta", clear
```
:::
...
- `r/_pkgdown.yml` — the family grouping rendered on the pkgdown site, driven by roxygen `@concept` tags.

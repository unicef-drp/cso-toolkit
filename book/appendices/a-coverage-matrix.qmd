...
| `dw_verbosity` | Yes (session verbose/debug switch) | Planned | n/a |
| `dw_default_unicef_allowlist` | Yes (remote-URL freeze allowlist) | Planned | n/a (no remote-URL reads in Stata) |
| `dw_map_drive` | n/a | n/a | Yes (`dw_map_drive.ado` — Stata-only onboarding: maps the team network drive from `dwZDrive`/`dwZDriveUNC`; the toolkit port of datalib's `mapzdrive`) |
...
| `dw_publish` (Helix submission) | Skeleton — exported but errors with guidance; the Helix endpoint integration is tracked in issue #15 | Planned | n/a (API-side capability) |
...
| Wired into CI | Yes (`r-check.yml`: Ubuntu release+devel, macOS, Windows) | Conformance driver only (`conformance.yml`); unit tests manual-only | No (Stata conformance driver pending license provisioning, issue #133) |
| Cross-language conformance gate | Yes (driver + comparator in `conformance.yml`) | Yes (driver + comparator in `conformance.yml`) | Pending (issue #133) |
...
## Related contracts outside this matrix

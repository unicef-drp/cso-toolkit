...
The repository headline version is **v0.5.0**, but that number is carried by the R package (`csotoolkit` 0.5.0) and the shared `NEWS.md`. The Python package still declares `__version__ = "0.4.0"`. Treat the languages as released on slightly different cadences: R leads, Python trails by a point release, and the Stata package versions per-command via its `.ado` files. Where a cell below says "Yes" for a recent feature, R is the sibling guaranteed to have it first.
:::
...
R and Python share the full `api=` dispatch surface — **ten** keys, identical on both sides: `uis`, `sdmx`, `sdmx_codelist`, `wb`, `wb_indicators`, `ilo`, `unsd_sdg`, `github_raw`, `http`, and `json_get`. For all ten, parity is complete.
...
- **R** is the reference implementation: every capability family, the full format set including `.rds` and gzipped CSV/TSV, the only generated reference site, and the only CI-gated test suite. It also carries the highest version (v0.5.0). If a behaviour is ambiguous, R defines it.
- **Python** is a near-complete functional port — core IO (including `.pkl`), all ten external-API dispatch keys, aggregation, vintage sync, and scaffolding — with full type hints, but no generated reference site, CI-manual-only testing, a few R-only helpers it has not yet picked up (`dw_stage`, `dw_root`, `dw_pop`, `dw_regions`), and a version (0.4.0) that trails R by a point release.

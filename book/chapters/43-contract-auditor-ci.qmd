...
As of writing, the Python package trails the R package — `python/pyproject.toml` declares `0.4.0` while `r/DESCRIPTION` declares `0.5.0`. If rule-set parity matters to you, pin the version you install and check that the Python registry carries the rules your contract relies on, rather than assuming the two languages catch exactly the same calls. The versioning-and-releases chapter explains how the two version lines are reconciled.
:::

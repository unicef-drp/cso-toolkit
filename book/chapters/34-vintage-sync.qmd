...
::: {.callout-warning}
The R and Python stamps are meant to track each other but can drift between releases: in the version this handbook documents, the R `dw_toolkit_version()` returns `"0.5.0"` while the Python port still returns `"0.4.0"`. The stamp is a hard-coded constant in each port (`_TOOLKIT_VERSION` in Python), so a release that bumps only one side leaves the other behind. Treat the value as a floor for the language you are calling it from, not as a cross-language equality check.
:::
...
* per-file `version` header (e.g. dw_save.ado is "version 1.1 (v0.4.0)").
...
| `dw_toolkit_version` | yes (`"0.5.0"`) | yes (`"0.4.0"`) | **none** (per-`.ado` `version` headers) |
